...
EVAL_SAVE_FILE = "./output/GLOBAL_MRR_IMDB.json"
...
@torch.no_grad()
def opt_eval_mrr_imdb(model, model_name, dev, save_title='UNNAMED_OPT_MRR_IMDB'):
    """
    Evaluate an OPT model using Mean Reciprocal Rank (MRR) on the IMDB test set.

    IMDB contains 25,000 test movie reviews (12,500 positive, 12,500 negative).
    We concatenate all test reviews and compute MRR on next-token prediction across
    the full text, measuring how well the model ranks the correct next token.

    This provides a different domain (movie reviews) compared to PTB (financial text)
    and AG News (news articles), testing generalization of quantization quality.
    """
    print("Evaluating MRR on IMDB test set ...")
    ...
    print(f"  IMDB test set: {test_ids.numel()} tokens, {nsamples} chunks of {seqlen}")
    ...
    print(f"MRR on IMDB: {mrr:.6f} ({total_positions} positions)")
