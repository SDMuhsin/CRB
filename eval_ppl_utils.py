...
@torch.no_grad()
def llama_eval(model, testenc, dev,  dataset: str, log_wandb: bool = False, save_title= 'UNNAMED_LLAMA', save=True):
...
@torch.no_grad()
def qwen_eval(model, testenc, dev, dataset: str, log_wandb: bool = False, save_title='UNNAMED_QWEN', save=True):
...
@torch.no_grad()
def granite_eval(model, testenc, dev, dataset: str, log_wandb: bool = False, save_title='UNNAMED_GRANITE', save=True):
    print("Evaluating (Granite) ...")
    ...
    # Granite uses logits_scaling (muP) — must apply it for correct PPL
    ...
    for i in range(nsamples):
        ...
        if model.model.norm is not None:
            hidden_states = model.model.norm(hidden_states)
        lm_logits = model.lm_head(hidden_states)
        lm_logits = lm_logits / logits_scaling
        shift_logits = lm_logits[:, :-1, :].contiguous()
        shift_labels = testenc[:, (i * model.seqlen) : ((i + 1) * model.seqlen)][:, 1:]
        loss_fct = nn.CrossEntropyLoss()
        loss = loss_fct(
...
@torch.no_grad()
def pythia_eval(model, testenc, dev, dataset: str, log_wandb: bool = False, save_title='UNNAMED_PYTHIA', save=True):
    print("Evaluating (Pythia/GPT-NeoX) ...")
...
@torch.no_grad()
def bloom_eval(model, testenc, dev, dataset: str, log_wandb: bool = False, save_title='UNNAMED_BLOOM', save=True):
    print("Evaluating (BLOOM) ...")
