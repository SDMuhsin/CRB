...
EVAL_SAVE_FILE = "./output/GLOBAL_MRR_AGNEWS.json"
...
@torch.no_grad()
def opt_eval_mrr_agnews(model, model_name, dev, save_title='UNNAMED_OPT_MRR_AGNEWS'):
    """
    Evaluate an OPT model using Mean Reciprocal Rank (MRR) on the AG News test set.

    AG News is a news classification dataset with 7,600 test articles across 4 categories
    (World, Sports, Business, Sci/Tech). We concatenate all test articles and compute MRR
    on next-token prediction across the full text, measuring how well the model ranks the
    correct next token in its output distribution.

    MRR captures ranking quality -- even when the top-1 prediction is wrong, a high MRR
    means the correct token is near the top. This complements PPL (log-prob) and accuracy
    (binary hit/miss) as an evaluation metric.
    """
    print("Evaluating MRR on AG News test set ...")
    ...
    testenc = tokenizer(all_text, return_tensors='pt')
    test_ids = testenc.input_ids  # [1, total_tokens]

    seqlen = model.seqlen
    nsamples = test_ids.numel() // seqlen

    print(f"  AG News test set: {test_ids.numel()} tokens, {nsamples} chunks of {seqlen}")
    ...
    print(f"MRR on AG News: {mrr:.6f} ({total_positions} positions)")

    model.cpu()
    torch.cuda.empty_cache()
    model.config.use_cache = use_cache

    save_result(save_title, mrr)
