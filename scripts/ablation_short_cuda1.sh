#!/usr/bin/env bash
# Partition × Codebook-bits ablation — SHORT (1h smoke test) — GPU 1.
#
# Runs 2 cells sequentially on cuda:1:
#   1. Qwen3-0.6B  partition=1  codebook_bits=3  (K=8, single-codebook DOML)
#   2. Qwen3-0.6B  partition=3  codebook_bits=3  (K=8, structural-partition DOML)
#
# Each cell: full quantization + 7-task eval (3 PPL + 4 downstream).
# Expected wall time: ~32 min/cell × 2 = ~65 min total.
#
# Output:
#   results/partition_K_ablation_qwen3_06b_short.csv    (shared with cuda:0 script — both write here)
#   logs/partition_K_ablation_short/cuda1_*.log         (per-cell logs + master log)
#
# Usage:
#   nohup bash scripts/ablation_short_cuda1.sh > logs/partition_K_ablation_short/cuda1_master.log 2>&1 &
#
...
GPU_ID=1
DEVICE="cuda:0"  # CUDA_VISIBLE_DEVICES restricts to one GPU; from Python's view it's cuda:0
