...
techniques=(
    ...
    # (jobs 13202955/13202956, full 7-task coverage). Asym variants TLE'd at
    # 8h (jobs 13202957/13202958 only reached 33/252 sublayers, 13%); see
    # walltime bump below.
...
get_time_limit() {
    ...
    case $1 in
        ...
        sdoml-s50)             echo "08:00:00" ;;  # b2 — same envelope as DOML (S10 mirror)
        sdoml-s20)             echo "08:00:00" ;;  # b2
        sdoml_part_asym-s50)   echo "60:00:00" ;;  # b4 — 2026-05-04 bump: jobs 13202957/13202958 TLE at 4h (13% complete); per-row loop is ~10× slower than S10 layer-0 estimate
        sdoml_part_asym-s20)   echo "60:00:00" ;;  # b4
        sparsegpt-s50)         echo "07:00:00" ;;  # b2 — measured: ~25 min quant (extrapolate from leanquant 4B 36 min A40 × MIG factor) + ~3 h eval (FLOP-scaled from 0.6B 40 min × 6.67 / 1.5 = 178 min). Estimate 3.4 h × 1.75 margin = 5.9 h; bump to 7 h for headroom over calibration uncertainty.
        sparsegpt-s50-w4)      echo "07:00:00" ;;  # b2
        sparsegpt-s50-w2)      echo "07:00:00" ;;  # b2
        sparsegpt-2:4-w4)      echo "07:00:00" ;;  # b2
        *)            echo "06:00:00" ;;
    esac
}
