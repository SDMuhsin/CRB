...
EVAL_SAVE_FILE = "./output/GLOBAL_MRR_YELP.json"
...
@torch.no_grad()
def opt_eval_mrr_yelp(model, model_name, dev, save_title='UNNAMED_OPT_MRR_YELP'):
    """
    Evaluate an OPT model using Mean Reciprocal Rank (MRR) on the Yelp Review Full test set.

    Yelp Review Full contains 50,000 test reviews across 5 star ratings (1-5 stars),
    covering restaurants, businesses, and services. We concatenate all test reviews and
    compute MRR on next-token prediction across the full text, measuring how well the
    model ranks the correct next token in its output distribution.

    This provides a different review domain (restaurants/businesses) compared to IMDB
    (movie reviews), AG News (news articles), and PTB (financial text).
    """
    print("Evaluating MRR on Yelp Review Full test set ...")
    ...
    print(f"  Yelp test set: {test_ids.numel()} tokens, {nsamples} chunks of {seqlen}")
    ...
    print(f"MRR on Yelp: {mrr:.6f} ({total_positions} positions)")
